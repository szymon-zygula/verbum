...
use verbum::{
    ...
    rewriting::rule::Rule,
    string_rewriting::{to_string_language, expression_to_paths, rule_to_induced_rules},
};
...
fn main() {
    ...
    // Define the topology (arity of each symbol)
    let mut topology = HashMap::new();
    topology.insert(0, 2); // + has arity 2
    topology.insert(1, 2); // * has arity 2
    topology.insert(2, 1); // sin has arity 1
    topology.insert(3, 0); // x has arity 0
    ...
    for id in 0..lang.symbol_count() {
        ...
        let arity = topology.get(&id).copied().unwrap_or(0);
    ...
    let induced_rules2 = rule_to_induced_rules(&rule2, &lang, &string_lang, &topology);
